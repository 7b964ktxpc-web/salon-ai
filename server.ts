...
async function startServer() {
  ...
  app.get('/api/settings', (_req, res) => res.json({ settings: db.businessSettings, workingHours: db.workingHours, subscription: db.getSubscription(), activePlan: db.getActivePlan(), yandexStatus: yandexGpt.getStatus() }));
  ...
  app.get('/api/subscription', (_req, res) => res.json({ subscription: db.getSubscription(), activePlan: db.getActivePlan(), plans: db.getSubscriptionPlans() }));
  ...
  app.post('/api/subscription/autorenew', (req, res) => res.json({ success: true, subscription: db.toggleSubscriptionAutoRenew(Boolean(req.body.autoRenew)) }));

  app.get('/api/platform/stats', (_req, res) => res.json(db.getPlatformStats()));
  app.get('/api/platform/salons', (_req, res) => res.json({ salons: db.getAllSalons(), activeSalonId: db.activeSalonId, plans: db.getSubscriptionPlans() }));
  ...
  app.get('/api/platform/broadcasts', (_req, res) => res.json({ broadcasts: db.getBroadcasts() }));
  app.post('/api/platform/switch-salon', (req, res) => {
    if (!req.body.salonId) return res.status(400).json({ error: 'ID салона не указан' });
    const salon = db.setActiveSalon(req.body.salonId);
    res.json({ success: true, activeSalonId: db.activeSalonId, salon, settings: salon.settings, services: salon.services, subscription: salon.subscription });
  });
  ...
  app.get('/api/masters/presets', (_req, res) => res.json({ presets: db.getStudioPresets(), currentSettings: db.businessSettings }));
  ...
  app.get('/api/dialogs', (_req, res) => res.json(db.getConversations()));
  ...
  app.get('/api/notifications', (req, res) => res.json(db.getNotifications(req.query.clientId as string | undefined)));
  ...
  app.put('/api/notifications/:id/read', (req, res) => res.json({ success: db.markNotificationRead(req.params.id) }));
  app.post('/api/notifications/:id/confirm', (req, res) => res.json({ success: db.confirmAppointmentFromNotification(req.params.id) }));

  app.get('/api/discounts', (_req, res) => res.json(db.getDiscountCampaigns()));
  ...
  app.post('/api/discounts/:id/broadcast', (req, res) => res.json(db.broadcastCampaign(req.params.id)));
  app.get('/api/notification-settings', (_req, res) => res.json(db.notificationSettings));
  app.put('/api/notification-settings', (req, res) => res.json(db.updateNotificationSettings(req.body)));
  ...
  app.post('/api/auth/admin-verify', (req, res) => {
    ...
    if (check.salon && !check.isSuperAdmin) db.setActiveSalon(check.salon.id);
    res.json({ success: true, isSuperAdmin: check.isSuperAdmin, masterName: check.isSuperAdmin ? 'Главный Администратор Платформы' : check.salon?.ownerName || 'Мастер', adminTelegramId: req.body.telegramId, salon: check.salon, salonId: check.salon?.id });
  });
